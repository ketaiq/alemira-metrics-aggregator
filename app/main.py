...
def aggregate_normal_metrics():
    ...
    gcloud_folders.sort()
    for folder in gcloud_folders:
        print(f"Processing {folder} ...")
        gcloud_aggregator = GCloudAggregator(
            NORMAL_GCLOUD_METRICS_PATH,
            folder,
            METRIC_TYPE_MAP_PATH,
            folder.lstrip("gcloud_metrics"),
        )
        gcloud_aggregator.merge_all_submetrics()
        gcloud_aggregator.aggregate_all_metrics()

    prometheus_folders = [
        folder for folder in os.listdir(NORMAL_PATH) if folder.startswith("day-")
    ]
    prometheus_folders.sort()
    for folder in prometheus_folders:
        print(f"Processing {folder} ...")
        exp_path = os.path.join(NORMAL_PATH, folder)
        prometheus_aggregator = PrometheusAggregator(
            exp_path, "metrics", TARGET_METRICS_PATH, f"-{folder}"
        )
        prometheus_aggregator.merge_all_submetrics()
        prometheus_aggregator.aggregate_all_metrics()
        ...
        locust_aggregator.aggregate_all_metrics()
    LocustAggregator.merge_normal_metrics(NORMAL_PATH)
...
def unify_prometheus_metrics():
    ...
    df_target_metrics = pd.read_csv(TARGET_METRICS_PATH)
...
def main():
    ...
    # gen_gcloud_target_metrics()
    # unify_prometheus_metrics()
    # unify_gcloud_metrics()
    # merge_normal_metrics()
    # merge_faulty_metrics_from_unified()
    merge_faulty_metrics_from_aggregated("day-8-linear-")
